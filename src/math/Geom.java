...
public class Geom {
	...
	public static double [] stats(ArrayList<Point> points){
		DoublyConnectedEdgeList el = new DoublyConnectedEdgeList();
		//LinkedHashMap<String, Double> s = new LinkedHashMap<String, Double>(); //hashmap for storing created stamps
		double [] targetVector = new double[11];
		//targetVector[0]=label;
		...
		if(width>height){
			scaleAmount = tarea/width;
		}
		else{
			scaleAmount = tarea/height;
		}
		//System.out.println("scale amount ="+scaleAmount);
		Point centroid = findAveragePoint(points);
		
		ArrayList<Point>scaledPoints = scale(scaleAmount,scaleAmount,centroid,points);
				
		for(int i=1;i<scaledPoints.size();i++){
					DCHalfEdge dcHE = new DCHalfEdge(scaledPoints.get(i-1),scaledPoints.get(i));
					el.addHalfEdge(dcHE);
		}
				
		
		double accumLength = accumLength(el);
		//s.put("accumLength",accumLength);
		targetVector[0]=accumLength;
		double normalLength = normalLength(el);
		//s.put("normalLength",normalLength);
		double accumAngle = accumAngle(el);
		//s.put("accumAngle",accumAngle);
		targetVector[1]=accumAngle;
		double accumRelativeAngle = accumRelativeAngle(el);
		//s.put("accumRelativeAngle",accumRelativeAngle);
		targetVector[2]=accumRelativeAngle;
		double accumSlope = accumSlope(el);
		//s.put("accumSlope",accumSlope);
		targetVector[3]=accumSlope;
		double numPoints = numPoints(el);
		//s.put("numPoints",numPoints);
		targetVector[4]=numPoints;
		double startEndDist = startEndDist(el);
		//s.put("startEndDist",startEndDist);
		targetVector[5]=startEndDist;
		double uArea = SignedPolygonArea(scaledPoints);
		//targetVector[7]=uArea;
		//s.put("uArea",uArea);
		double sWidth = getWidth(scaledPoints);
		targetVector[6]=sWidth;
		double sHeight = getHeight(scaledPoints);
		targetVector[7]=sHeight;

		double sArea = sWidth*sHeight;
		targetVector[8]=sArea;
		targetVector[9]=centroid.getX();
		targetVector[10]=centroid.getY();
...
public static ArrayList<Point> scale(double x, double y,  Point focus, ArrayList<Point> pts ) {
	 ArrayList<Point> points =  new ArrayList<Point>();
		for(int i=0;i<pts.size();i++){
			Point p = pts.get(i).copy();
	...
	public static double getWidth(ArrayList<Point> verticies){
		 ArrayList<Point> cP =  (ArrayList<Point>)verticies.clone();
		 Collections.sort(cP, new CmpX());
		 double leftX = cP.get(0).getX();
		 double rightX = cP.get(cP.size()-1).getX();
		 return rightX-leftX;
		
		
	}
	
	public static double getHeight(ArrayList<Point> verticies){
		 ArrayList<Point> cP =  (ArrayList<Point>)verticies.clone();
		 Collections.sort(cP, new CmpY());
		 double leftX = cP.get(0).getY();
		 double rightX = cP.get(cP.size()-1).getY();
		 return rightX-leftX;
		
		
	}
	
	public static Point findAveragePoint(ArrayList<Point> verticies){
		double sumX =0;
		double sumY = 0;
		for(int i=0;i<verticies.size();i++){
			sumX +=verticies.get(i).getX();
			sumY +=verticies.get(i).getY();

		}
		double avX = sumX/verticies.size();
		double avY = sumY/verticies.size();
		return new Point(avX,avY);
		
	}
	
	
	public static Point findCentroid(ArrayList<Point> verticies)
	{
		double cx=0,cy=0;
		double A= SignedPolygonArea(verticies);
		if (verticies.size()>0){
	...
	//returns total length of DCEdgelist
    public static  double accumLength(DoublyConnectedEdgeList dc){
    	ArrayList<DCHalfEdge> edges = dc.edges;
    	double length= 0;
    	for(int i=0;i<edges.size();i++){
    		DCHalfEdge edge = edges.get(i);
    		Vec2d vec = new Vec2d(edge.end.getX()-edge.start.getX(),edge.end.getY()-edge.start.getY());
	...
	//returns normal length of DCEdgelist
    public static  double normalLength(DoublyConnectedEdgeList dc){
    	ArrayList<DCHalfEdge> edges = dc.edges;
    	double length= 0;
    	for(int i=0;i<edges.size();i++){
    		DCHalfEdge edge = edges.get(i);
    		Vec2d vec = new Vec2d(edge.end.getX()-edge.start.getX(),edge.end.getY()-edge.start.getY());
    		vec = Vec2d.Normalize(vec);
    		double mag = vec.Length();
    		length+=mag;
    	...
    	if(length!=edges.size()){
    		System.out.println("WARNING, Normalized length is not equal to num of edges");
    	}
    	return length;
    }
    
    
  //returns accumulated angle of DCEdgeList
    public static  double accumAngle(DoublyConnectedEdgeList dc){
    	ArrayList<DCHalfEdge> edges = dc.edges;
    	double totalAngle= 0;
    	for(int i=0;i<edges.size();i++){
    		DCHalfEdge edge = edges.get(i);
    		Double angle = Geom.cartToPolar(edge.end.getX()-edge.start.getX(), edge.end.getY()-edge.start.getY())[1];
    		
    		//System.out.println("angle ="+angle);
    		if(angle>180){
    			angle = 180-angle;
    		}
    		//System.out.println("modified angle ="+angle);
    		totalAngle +=angle;
  ...
  //returns accumulated angle of DCEdgeList
    public static  double accumRelativeAngle(DoublyConnectedEdgeList dc){
    	ArrayList<DCHalfEdge> edges = dc.edges;
    	double totalAngle= 0;
    	for(int i=0;i<edges.size();i++){
    		DCHalfEdge edge = edges.get(i);
    ...
    //returns accumulated slope of DCEdgeList
      public static  double accumSlope(DoublyConnectedEdgeList dc){
      	ArrayList<DCHalfEdge> edges = dc.edges;
      	double totalSlope= 0;
      	for(int i=0;i<edges.size();i++){
      		DCHalfEdge edge = edges.get(i);
      ...
      public static int numPoints(DoublyConnectedEdgeList dc){
    	  return dc.edges.size()+1;
      }
      
      
      public static double startEndDist(DoublyConnectedEdgeList dc){
    	  Point start = dc.edges.get(0).start;
    	  Point end = dc.edges.get(dc.edges.size()-1).end;
...
*/
 ...
 //find area of a polygon
    public static double SignedPolygonArea(ArrayList<Point> points)
    {
